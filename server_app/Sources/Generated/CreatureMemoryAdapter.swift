...
public class CreatureMemoryAdapter: CreatureAdapter {
    var items: [String:Creature] = [:]

    public func findAll(onCompletion: @escaping ([Creature], Error?) -> Void) {
        onCompletion(items.map { $1 }, nil)
    }

    public func create(_ model: Creature, onCompletion: @escaping (Creature?, Error?) -> Void) {
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (Creature?, Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: Creature, onCompletion: @escaping (Creature?, Error?) -> Void) {
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (Creature?, Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
