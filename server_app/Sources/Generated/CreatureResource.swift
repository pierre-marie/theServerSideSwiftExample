...
public class CreatureResource {
    private let adapter: CreatureAdapter
    private let path = "/api/creatures"
    private let pathWithID = "/api/creatures/:id"

    init(factory: AdapterFactory) throws {
        adapter = try factory.getCreatureAdapter()
    }
    ...
    private func handleCreate(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) {
        ...
        do {
            let model = try Creature(json: json)
